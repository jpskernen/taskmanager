...
function addTask(
  ...
  if (taskTitles.has(title)) {
    addTaskError.textContent = "A task with this title already exists.";
    addTaskError.style.display = "block";
    return;
  }
...
function saveTasksToStorage() {
  ...
  showStatus("Tasks saved to localStorage successfully!");
}
...
function loadTasksFromStorage(renderCallback, updateCountCallback) {
  ...
  if (!storedTasks) {
    showStatus("No saved tasks found in localStorage.", "info");
    return;
  }
  ...
  // Restore taskId counter
  if (storedTaskId !== null) {
    taskId = JSON.parse(storedTaskId);
  }

  // Rebuild taskMap and taskTitles from loaded tasks
  taskMap.clear();
  taskTitles.clear();
  tasks.forEach((task) => {
  ...
  showStatus(`Loaded ${tasks.length} task(s) from localStorage successfully!`);
}
